...
Future<void> toggleListening(BuildContext context,Completer<GoogleMapController> _controller, MapServices services) async {

  var word = await s.sttFn(['yes', 'no', 'home','fci', 'list']);
  print("the function has returned ${word.toString()}");
  if(word.toString() == 'yes'){
    TextSpeech.speak("thank you for your feedback");
  }
  if(word.toString() == 'start navigation'){
